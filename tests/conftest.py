...
def api_key_header() -> dict:
    """Exposed as a fixture rather than a module constant: tests/ has no
    __init__.py, so `from tests.conftest import ...` only resolves when the
    project root happens to be on sys.path (e.g. `python -m pytest`) and breaks
    under a plain `pytest` invocation."""
